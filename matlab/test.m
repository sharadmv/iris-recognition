...
metric
size(centers)
drawCircle(centers(1,1),centers(1,2),radii(1,1));
pupil_radius = radii(1,1);
pupil_center = [centers(1,1) centers(1,2)];



BW3 = edge(gray,'canny',.1);
...
for i=1:72
    ...
    while hit_edge == 0 && floor(ray_pts(i,1)) < size(BW4,1) &&  floor(ray_pts(i,2)) < size(BW4,2)
        hit_edge = BW4(floor(ray_pts(i,1)),floor(ray_pts(i,2))) || BW4(floor(ray_pts(i,1))+sign(ray_dir(1)),floor(ray_pts(i,2))) || BW4(floor(ray_pts(i,1)),floor(ray_pts(i,2))+sign(ray_dir(2)));
        ray_pts(i,:) = ray_pts(i,:)+ray_dir;
    end
    drawCircle(ray_pts(i,1),ray_pts(i,2),1);
end
